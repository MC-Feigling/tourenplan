...
function orsAuthHeaders(apiKey: string): Record<string, string> {
  return {
...
export async function geocodeAddress(query: string): Promise<GeocodeResult[]> {
  ...
  let data: { features?: OrsGeocodeFeature[] }
  try {
    data = await ofetch<{ features?: OrsGeocodeFeature[] }>(url.toString(), {
      headers: orsAuthHeaders(apiKey),
    })
  } catch (error: unknown) {
    const fetchError = error as { status?: number; statusCode?: number; data?: string; message?: string }
    const status = fetchError.status ?? fetchError.statusCode ?? 502
    const detail = typeof fetchError.data === 'string' ? fetchError.data : fetchError.message
    throwOrsError(status, 'geocoding', detail)
  }
...
export async function fetchDirections(
  ...
  try {
    data = await ofetch<OrsDirectionsResponse>(url, {
      method: 'POST',
      headers: {
    ...
    const status = fetchError.status ?? fetchError.statusCode ?? 502
    const detail = typeof fetchError.data === 'string' ? fetchError.data : fetchError.message
    throwOrsError(status, 'routing', detail)
  }
