...
export function useStaffApi() {
  ...
  async function list() {
    ...
    error.value = null
    try {
      return await $fetch<{ items: PublicStaffMember[] }>('/api/staff', { credentials: 'include' })
    } catch (e: unknown) {
  ...
  async function get(id: string) {
    ...
    error.value = null
    try {
      return await $fetch<{ item: PublicStaffMember; leaveRequests: PublicLeaveRequest[] }>(
        `/api/staff/${id}`,
        { credentials: 'include' },
      )
    } catch (e: unknown) {
  ...
  async function create(payload: StaffFormState) {
    ...
    error.value = null
    try {
      return await $fetch<{ item: PublicStaffMember }>('/api/staff', {
        method: 'POST',
        body: payload,
        credentials: 'include',
      })
  ...
  async function update(id: string, payload: Partial<StaffFormState>) {
    ...
    error.value = null
    try {
      return await $fetch<{ item: PublicStaffMember }>(`/api/staff/${id}`, {
        method: 'PATCH',
        body: payload,
        credentials: 'include',
      })
  ...
  async function addLeave(staffId: string, payload: LeaveFormState) {
    return await $fetch<{ item: PublicLeaveRequest }>(`/api/staff/${staffId}/leave`, {
      method: 'POST',
      body: payload,
      credentials: 'include',
    })
  ...
  async function removeLeave(id: string) {
    await $fetch(`/api/leave/${id}`, { method: 'DELETE', credentials: 'include' })
  }
