...
const drivers = ref<AssignmentDriverContext[]>([])
const vehicles = ref<AssignmentVehicleContext[]>([])
...
try {
  const resources = await assignmentApi.loadResources(form.value.date, form.value.date)
    ...
    <div class="surface-card space-y-4 p-5">
      <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <button
          ...
          :disabled="routing.loading || geocodedCount < 2"
          @click="onCalculateRoute"
        >
          {{ routing.loading ? 'Berechne…' : 'Route berechnen' }}
        </button>
      </div>
