...
<template>
  ...
  <div class="space-y-6">
    ...
    <div class="flex items-center gap-2">
      ...
      <button type="button" class="btn-ghost !min-h-10 !px-3" aria-label="Vorherige Woche" @click="week.shiftWeek(-1)">←</button>

      <button type="button" class="btn-ghost !min-h-10 flex-1 text-xs" @click="week.goToToday()">Heute</button>

      <button type="button" class="btn-ghost !min-h-10 !px-3" aria-label="Nächste Woche" @click="week.shiftWeek(1)">→</button>
    ...
    >
      ...
      {{ driverApiError ?? 'Touren konnten nicht geladen werden' }}
    ...
    <template v-else>
      ...
      <div v-if="!tours.length" class="surface-card p-6 text-center">
